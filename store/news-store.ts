  ...
  persist(
    (set, get) => ({
      ...
      fetchArticles: async (category = "", limit = 10, offset = 0) => {
        ...
        try {
          ...
          const res = await fetch(
            `${BASE_URL}/news?category=${category.toLowerCase()}&limit=${limit}&offset=${offset}`,
            {
              headers: {
                'token': `${token}`,
      ...
      bookmarkArticle: async(articleId: string) => {
        ...
        try {
          const res = await fetch(`${BASE_URL}/news/save/${articleId}`, {
            method: 'POST',
            headers: {
              token: `${token}`
            }
      ...
      likeArticle: async (articleId: string) => {
        ...
        try {
          const res = await fetch(`${BASE_URL}/news/like/${articleId}`, {
            method: 'POST',
            headers: {
              token: `${token}`
            }
      ...
      getBookmarkedArticles: async() => {
        try {
          const token = useAuthStore.getState().token;
          const response = await fetch(`${BASE_URL}/news/saved`,{
            headers: {
              token: `${token}`
            }
          })
